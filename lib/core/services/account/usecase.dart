...
//creates users virtual account upon registration of an account
  ...
  Future<ResponseModel<ResolvedAcctModel>> resolveBankAcct(
      Map<String, dynamic> payload) async {
    Response response =
        await accountRepoImplementation.resolveBankAcct(payload);
    int statusCode = response.statusCode ?? 000;
    debugPrint(
        'DDDDDDDDDDDDDDDDDDDDDDDDDDDDDstatus: [INFO] ${response.statusCode}');
    debugPrint('DDDDDDDDDDDDDDDDDDDDDDDDDDDDDdata: [INFO] ${response.data}');
    ...
    if (statusCode >= 200 && statusCode <= 300) {
      return ResponseModel(
        status: true,
        message: response.statusMessage,
        data: ResolvedAcctModel.fromJson(response.data["data"]),
      );
    }
